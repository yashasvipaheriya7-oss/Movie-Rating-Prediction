...
st.markdown("""
    ...
    /* overflow-y: scroll keeps the scrollbar always visible so the page doesn't jump! */
    html, body, .stApp { 
    ...
    .full-width-footer {
        width: auto; margin-left: -5rem; margin-right: -5rem; /* Fixes the 100vw bug that causes shaking */
    ...
    .modal-close:hover {
        background: rgba(0,255,255,0.15); color: #00FFFF !important; transform: rotate(90deg);
    }
...
# --- CALLBACK TO SAFELY UPDATE SEARCH BAR WITHOUT CRASHING ---
...
if not st.session_state['auth']:
    ...
    with center:
        ...
        st.markdown("<h2 style='font-size:26px; margin-top:20px;'>Member Access</h2>", unsafe_allow_html=True)
        # FIXED: Added autocomplete="email" here
        ...
        if st.button("Sign In", use_container_width=True):
            if re.match(r"[^@]+@[^@]+\.[^@]+", email):
                ...
                st.rerun()
            else: st.error("Enter a valid email.")
else:
    user_color = get_user_color(st.session_state['user_email'])
    
    # --- FIXED: USER ICON CSS FIX ---
    st.markdown(f"""
        <style>
        /* Force the popover button to be a perfect circle */
        ...
        /* Ensure all inner wrappers are transparent and text is forced to black */
        div[data-testid="stPopover"] button div, 
        div[data-testid="stPopover"] button p,
        div[data-testid="stPopover"] button span {{
            background-color: transparent !important; 
            color: #000000 !important;
            -webkit-text-fill-color: #000000 !important; /* Overrides Streamlit Dark Mode */
            font-weight: 800 !important; font-size: 20px !important; 
            margin: 0 !important; padding: 0 !important;
            display: flex !important; align-items: center !important; justify-content: center !important;
        }}
        
        /* Hide the dropdown chevron SVG arrow */
        div[data-testid="stPopover"] button svg {{ display: none !important; }}
        
        /* Style the actual popup window */
        div[data-testid="stPopoverBody"] {{ background-color: #FFFFFF !important; border-radius: 15px !important; }}
        div[data-testid="stPopoverBody"] p {{ color: #000000 !important; -webkit-text-fill-color: #000000 !important; }}
        </style>
    """, unsafe_allow_html=True)
    ...
    with h2:
        ...
        with st.popover(u_init):
            st.markdown(f'<div style="text-align:center; padding:5px; background-color:#FFFFFF;">'
                        f'<p style="font-size:14px; color:#71717A !important; margin-bottom: 20px; font-weight: 500;">{st.session_state["user_email"]}</p>'
                        f'<div style="background:{user_color}; color:#FFFFFF !important; -webkit-text-fill-color: #FFFFFF !important; width:70px; height:70px; border-radius:50%; margin:0 auto 15px; display:flex; align-items:center; justify-content:center; font-size:30px; font-weight:900;">{u_init}</div>'
                        f'<p style="font-size:22px; color:#000000 !important; font-weight:500; margin-bottom: 20px;">Hi, User!</p></div>', unsafe_allow_html=True)
    ...
    if final_query:
        ...
        if data:
            ...
            with t_t:
                ...
                # FIXED: The original code had an incomplete string here `if video: st.video(f"`
