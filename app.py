...
st.markdown("""
    ...
    /* ══════════════════════════════════════════════
       NUCLEAR INPUT FIX — NO BORDER, DARK SEAMLESS
    ══════════════════════════════════════════════ */
    .stTextInput > div,
    .stTextInput > div > div,
    ...
    /* Search bar icon */
    [data-baseweb="input"]:has(input[placeholder="Search for a movie title..."])::after {
        content: "";
    ...
    .modal-content {
        ...
        box-shadow: 0 20px 50px rgba(0,0,0,0.8); transition: top 0.4s cubic-bezier(0.25, 0.8, 0.25, 1);
    }
    ...
    .tech-tag { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); padding: 6px 14px; border-radius: 8px; font-size: 14px; color: #FFFFFF; font-weight: 600; }
...
def get_movie_details(query):
    ...
    try:
        res = session.get("https://api.tmdb.org/3/search/movie", params={"api_key": TMDB_API_KEY, "query": query}, headers=headers, timeout=10)
        if res.status_code == 401: return None, "❌ Invalid TMDB API Key."
        res.raise_for_status()
        data = res.json()
        if not data.get('results'): return None, "Movie not found."
        mid = data['results'][0]['id']
        detail_res = session.get(f"https://api.tmdb.org/3/movie/{mid}", params={"api_key": TMDB_API_KEY, "append_to_response": "credits,videos,recommendations"}, headers=headers, timeout=10)
        detail_res.raise_for_status()
        return detail_res.json(), None
    except requests.exceptions.ConnectionError:
        return None, "🌐 Network Error: Try using a VPN like Cloudflare WARP (1.1.1.1)!"
...
if not st.session_state['auth']:
    st.markdown("""
        ...
        [data-testid="stAppViewContainer"] {
            background-image: linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.9)),
                url('https://assets.nflxext.com/ffe/siteui/vlv3/a73c4363-1dcd-4719-b3b1-3725418fd91d/fe1147dd-78be-44aa-a0e5-2d2994305a13/IN-en-20231016-popsignuptwoweeks-perspective_alpha_website_large.jpg') !important;
        ...
        /* ══ LOGIN INPUTS: BORDERLESS DARK SEAMLESS ══ */
        div[data-testid="column"]:nth-child(2) .stTextInput > div,
        div[data-testid="column"]:nth-child(2) .stTextInput > div > div,
        div[data-testid="column"]:nth-child(2) .stTextInput > div > div > div,
        div[data-testid="column"]:nth-child(2) [data-baseweb="input"],
        div[data-testid="column"]:nth-child(2) [data-baseweb="input"] > div {
        ...
        div[data-testid="column"]:nth-child(2) .stTextInput > div:focus-within,
        div[data-testid="column"]:nth-child(2) [data-baseweb="input"]:focus-within,
        div[data-testid="column"]:nth-child(2) [data-baseweb="input"] > div:focus-within,
        div[data-testid="column"]:nth-child(2) .stTextInput input:focus,
        div[data-testid="column"]:nth-child(2) [data-baseweb="input"] input:focus {
            border: none !important;
            outline: none !important;
            box-shadow: none !important;
        }
        ...
        div[data-testid="column"]:nth-child(2) [data-baseweb="input"] input::placeholder {
            color: #888888 !important;
            -webkit-text-fill-color: #888888 !important;
            opacity: 1 !important;
        }
        div[data-testid="column"]:nth-child(2) .stTextInput input:-webkit-autofill,
        div[data-testid="column"]:nth-child(2) .stTextInput input:-webkit-autofill:focus {
            -webkit-box-shadow: 0 0 0px 1000px #2A2A2A inset !important;
            -webkit-text-fill-color: #FFFFFF !important;
            border: none !important;
        }
        div[data-testid="column"]:nth-child(2) [data-baseweb="input"]::after { display: none !important; }

        button[kind="primary"] {
            background-color: #E50914 !important; background-image: none !important;
        ...
        button[kind="primary"]:hover { background-color: #C11119 !important; }
        div[data-testid="stCheckbox"] label p { color: #8C8C8C !important; font-size: 14px !important; }
        </style>
    """, unsafe_allow_html=True)
    ...
    with center:
        ...
        with c2:
            st.markdown("<div style='text-align:right; margin-top:8px;'><a href='mailto:support@cinepredict.com' style='color:#8C8C8C; text-decoration:none; font-size:14px;'>Need help?</a></div>", unsafe_allow_html=True)
        ...
        st.markdown("<div style='margin-top:60px; color:#737373; font-size:16px;'>New to Cine Predict? <span style='color:white; font-weight:500; cursor:pointer;'>Sign up now.</span></div>", unsafe_allow_html=True)
    ...
    st.markdown(f"""
        ...
        div[data-testid="stPopover"] button {{
            ...
            width: 100% !important; height: 45px !important; border: 2px solid #00FFFF !important;
            padding: 0 20px !important; box-shadow: 0px 0px 10px rgba(0,255,255,0.1) !important;
        ...
        div[data-testid="stPopover"] button:hover {{
            ...
            box-shadow: 0px 0px 15px rgba(0,255,255,0.3) !important; transform: translateY(-2px);
        }}
        div[data-testid="stPopover"] button div,
        div[data-testid="stPopover"] button p,
        div[data-testid="stPopover"] button span {{
            background-color: transparent !important; color: #00FFFF !important;
            -webkit-text-fill-color: #00FFFF !important; font-weight: 700 !important; font-size: 16px !important;
            margin: 0 !important; padding: 0 !important;
            display: flex !important; align-items: center !important; justify-content: center !important; gap: 6px !important;
        }}
        div[data-testid="stPopover"] button svg {{ display: none !important; }}
        div[data-testid="stPopoverBody"] {{
            background-color: #1A1A1D !important; border: none !important; outline: none !important;
            box-shadow: none !important; border-radius: 15px !important; padding: 20px !important;
        }}
        div[data-testid="stPopoverBody"] .stButton > button {{
            ...
            height: 45px !important; font-size: 16px !important; font-weight: 500 !important;
            margin-top: 15px !important; box-shadow: none !important;
        }}
        ...
        with st.popover("👤 Profile"):
            st.markdown(f'''
                <div style="text-align:center; padding-top:10px;">
                    <p style="font-size:15px; color:#888888 !important; -webkit-text-fill-color:#888888 !important; margin-bottom:25px; margin-top:0; font-weight:400;">{st.session_state["user_email"]}</p>
                    <div style="background:{user_color}; color:#FFFFFF !important; -webkit-text-fill-color:#FFFFFF !important; width:90px; height:90px; border-radius:50%; margin:0 auto 20px; display:flex; align-items:center; justify-content:center; font-size:42px; font-weight:bold;">{u_init}</div>
                    <p style="font-size:24px; color:#000000 !important; -webkit-text-fill-color:#000000 !important; font-weight:500; margin-bottom:10px;">Hi, User!</p>
                </div>
            ''', unsafe_allow_html=True)
