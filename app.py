...
st.markdown("""
    ...
    /* ============================================================
       FIX 1: INPUT FIELDS — black box + invisible text fix
       Root cause: Streamlit Cloud injects a dark theme that sets
       input backgrounds to near-black (#262730) with dark text.
       We override every layer of the component tree with !important
       and use the webkit inset shadow trick to beat autofill.
    ============================================================ */

    /* Layer 1 — BaseWeb container wrappers */
    ...
    /* Layer 2 — The actual <input> element */
    input,
    input[type="text"],
    input[type="email"],
    input[type="search"],
    ...
    .stTextInput input {
        ...
        /* The inset box-shadow overrides browser autofill yellow/dark bg */
    ...
    /* Layer 3 — Autofill states (browser override) */
    ...
    /* ============================================================
       FIX 2: REMOVE profile/popover icon styling entirely.
       The popover button is not rendered in the fixed version,
       but these rules are removed so no stray icon appears.
    ============================================================ */

    /* FIX: Remove white border/outline from the popover container */
    ...
    /* Popover body — dark background, no white border */
    div[data-testid="stPopoverBody"] {
    ...
    /* ============================================================
       FIX 3: MOBILE RESPONSIVE OVERRIDES
       Target ≤768px (tablets) and ≤480px (phones) separately.
    ============================================================ */
    @media screen and (max-width: 768px) {
        /* Reduce block container padding on tablets */
        .block-container {
    ...
    @media screen and (max-width: 640px) {
        /* Stack all horizontal blocks vertically on phones */
        ...
        /* Login page: less top space */
        .login-spacer { margin-top: 10px !important; }

        /* Reduce block padding to maximize usable width */
        .block-container {
            padding-left: 0.5rem !important;
            padding-right: 0.5rem !important;
        }

        /* Footer margins flush with edges */
        .full-width-footer {
        ...
        /* Footer buttons wrap and shrink */
        .footer-btn {
...
if not st.session_state['auth']:
    # LOGIN PAGE — exactly as original: centered layout, no background image, no profile icon
    ...
    # MAIN APP — header row: logo only (NO profile/popover icon)
    render_logo(size_scale=0.8)

    # Sign out tucked away in sidebar to avoid cluttering the header
    with st.sidebar:
        st.markdown(
            f"<p style='color:#A1A1AA; font-size:13px; margin-bottom:8px;'>"
            f"Signed in as<br><strong style='color:#FFFFFF;'>{st.session_state['user_email']}</strong></p>",
            unsafe_allow_html=True
        )
