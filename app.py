...
# --- 1. CONFIGURATION ---
# SECURE API KEY SETUP (Reads from Streamlit Cloud Secrets)
try:
    TMDB_API_KEY = st.secrets["TMDB_API_KEY"]
except:
    # Fallback just in case you test locally without secrets file
...
st.markdown("""
    ...
    /* FIXED: Hide 'Press Enter to apply' so it stops overlapping the password eye icon */
    ...
    /* FIXED: SIMILAR SECTION TITLES (Restored to Transparent, removed blue gradient) */
    div[data-testid="stHorizontalBlock"] div[data-testid="stButton"] button {
        background: transparent !important; background-image: none !important; border: none !important; 
        padding: 0 !important; width: 100%; text-align: left !important; box-shadow: none !important;
    }
...
if not st.session_state['auth']:
    ...
    st.markdown("""
        <style>[data-testid="stAppViewContainer"] {
            background-image: linear-gradient(rgba(0, 0, 0, 0.6), rgba(0, 0, 0, 0.95)), url('https://assets.nflxext.com/ffe/siteui/vlv3/a73c4363-1dcd-4719-b3b1-3725418fd91d/fe1147dd-78be-44aa-a0e5-2d2994305a13/IN-en-20231016-popsignuptwoweeks-perspective_alpha_website_large.jpg') !important;
        ...
        /* Ensures the text inside the Netflix login boxes is pure white and visible on mobile */
        div[data-testid="column"]:nth-child(2) input {
            color: #FFFFFF !important;
            -webkit-text-fill-color: #FFFFFF !important;
            caret-color: white !important;
            padding-right: 40px !important; /* FIXED: Removed 140px so text shows on phones */
        ...
        /* Hide Search icon inside login fields */
        div[data-testid="column"]:nth-child(2) div[data-baseweb="input"]::after { display: none !important; }

        /* Login Button */
        button[kind="primary"] {
            background-color: #E50914 !important; background-image: none !important; color: white !important;
            border: none !important; border-radius: 4px !important; font-size: 16px !important; font-weight: 700 !important;
            padding: 12px !important; margin-top: 25px !important; margin-bottom: 10px !important;
        }
        button[kind="primary"]:hover { background-color: #C11119 !important; }
        div[data-testid="stCheckbox"] label p { color: #B3B3B3 !important; font-size: 14px !important; }
        
        /* Mobile padding adjustments for the login box */
    ...
    with center:
        ...
        # --- REMEMBER ME & HELP OPTIONS PLACED ABOVE THE BUTTON ---
        c1, c2 = st.columns([1, 1])
        with c1: st.checkbox("Remember me", value=True)
        with c2:
            st.markdown("<div style='text-align:right; margin-top:8px;'><a href='mailto:support@cinepredict.com' style='color:#B3B3B3; text-decoration:none; font-size:14px;'>Need help?</a></div>", unsafe_allow_html=True)
    ...
    st.markdown(f"""
        ...
        div[data-testid="stPopover"] button {{
            background-color: transparent !important; border-radius: 30px !important; width: 100% !important; height: 45px !important;
            border: 2px solid #00FFFF !important; padding: 0 20px !important; 
        ...
        div[data-testid="stPopover"] button div, div[data-testid="stPopover"] button p, div[data-testid="stPopover"] button span {{
            background-color: transparent !important; color: #00FFFF !important; font-weight: 700 !important; font-size: 16px !important; 
            margin: 0 !important; padding: 0 !important; gap: 6px !important;
        }}
        div[data-testid="stPopover"] button svg {{ display: none !important; }}
        
        div[data-testid="stPopoverBody"] {{ background-color: #FFFFFF !important; border-radius: 15px !important; padding: 20px !important; }}
        div[data-testid="stPopoverBody"] .stButton > button {{
            background: linear-gradient(to right, #6FE3CB, #2CB3E1) !important; color: #000000 !important; border: none !important;
            border-radius: 10px !important; width: 100% !important; height: 45px !important; font-weight: 500 !important; margin-top: 15px !important;
        }}
        </style>
    """, unsafe_allow_html=True)
        ...
        with st.popover("👤 Profile"):
            st.markdown(f'''
                <div style="text-align:center; padding-top:10px;">
                    <p style="font-size:15px; color:#888888 !important; margin-bottom: 25px; margin-top: 0;">{st.session_state["user_email"]}</p>
                    <div style="background:{user_color}; color:#FFFFFF !important; width:90px; height:90px; border-radius:50%; margin:0 auto 20px; display:flex; align-items:center; justify-content:center; font-size:42px; font-weight:bold;">{u_init}</div>
                    <p style="font-size:24px; color:#000000 !important; font-weight:500; margin-bottom: 10px;">Hi, User!</p>
                </div>
            ''', unsafe_allow_html=True)
    ...
    if final_query:
        ...
        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin-top: 100px;">
            ...
            <h1 style="font-family: 'Inter', sans-serif; font-weight: 800; font-size: 42px; margin-bottom: 10px; color: #FFFFFF; text-align:center;">Discover Your Next Favorite</h1>
